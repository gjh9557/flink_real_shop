...
//显示的定义time类型
    ...
    val stream=env.readTextFile("E:\\qqq\\days\\12.20\\real_shop\\HotltemsAnalysis\\src\\main\\resources\\UserBehavior.csv")
      .map(line=>{
        val arr = line.split(",")
...
//自定义实现process function  这里KeyedProcessFunction里面的三个类型分别是k,i,o就是上面的keyby那个类型，input,和output.
